...
import json, os, base64, hashlib, hmac, sqlite3, calendar, re, io, zipfile
...
def orders_page(request: Request, q: Optional[str] = None,
    ...
    # 只依班別排序（不再依單號）；同日以 id（新增先後）呈現；日期新到舊
    sql += " ORDER BY odt DESC, CASE WHEN shift='早班' THEN 0 ELSE 1 END, id ASC"
    with _conn() as c:
        rows = [dict(r) for r in c.execute(sql, params)]
        for i, r in enumerate(rows): r["idx"] = i  # 用於回復原排序
...
def orders_create(request: Request, odt: str = Form(...), shift: str = Form(...),
    ...
    with _conn() as c:
        ...
        c.commit()
    return RedirectResponse("/orders#focus-order-no", status_code=303)
...
def ai_analyze(request: Request, q: str = Form(""), mode: str = Form("auto"),
    ...
    def period_for(mode: str, start: str, end: str) -> Tuple[str,str,str]:
        if mode == "day":
            frm = end = _today(); return frm, end, "當日"
        if mode == "month":
            d = _parse_dt(_today()); frm = d.replace(day=1).isoformat()
            from calendar import monthrange; end = d.replace(day=monthrange(d.year,d.month)[1]).isoformat()
            return frm, end, "本月"
        if mode == "year":
